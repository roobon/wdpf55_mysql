...
--
-- Stand-in structure for view `custom_orders_view`
-- (See below for the actual view)
--
CREATE TABLE `custom_orders_view` (
...
(1001, 'Nadia', 'Trisha', '', 'tisha@gmail.com', '', NULL, 'web developer'),
(1002, 'Ahmed', 'Masum', 'x5800', 'masum@gmail.com', '1', NULL, 'President'),
...
--
-- Structure for view `custom_orders_view`
--
DROP TABLE IF EXISTS `custom_orders_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `custom_orders_view`  AS SELECT `customers`.`customerNumber` AS `customerNumber`, `customers`.`customerName` AS `Customer Name`, `customers`.`phone` AS `phone`, `customers`.`city` AS `city`, sum(`orderdetails`.`quantityOrdered`) AS `Total orders`, sum(`orderdetails`.`quantityOrdered` * `orderdetails`.`priceEach`) AS `Total Price` FROM (((`customers` join `orders`) join `products`) join `orderdetails`) WHERE `customers`.`customerNumber` = `orders`.`customerNumber` AND `orders`.`orderNumber` = `orderdetails`.`orderNumber` AND `orderdetails`.`productCode` = `products`.`productCode` GROUP BY `customers`.`customerNumber` ;
